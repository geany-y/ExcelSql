    ...
    getAllUser() {
        return new Promise((resolve, reject) => {
            const sql = `SELECT
                    ...
                    U.deleted
                FROM (((((
                    [Rooms$] AS R INNER JOIN [RoomsMaster$] AS RM ON R.rooms_id = RM.id)
                LEFT JOIN
                    [Users$] AS U ON R.users_id = U.id)
                LEFT JOIN
                    [UserStateMaster$] AS UM ON U.state = UM.id)
                LEFT JOIN
                    [Tenants$] AS T ON R.tenants_id = T.id)
                LEFT JOIN
                    [Groups$] AS G ON R.rooms_id = G.room_id)
                LEFT JOIN
                    [Groups_Master$] AS GM ON G.group_id = GM.id`;
            try {
                const res = this.execSql(sql);
                resolve(res);
            } catch (e) {
